...
input double TP1_Width_To_Launch_Times = 2.0;
input double TP2_Width_To_Launch_Times = 4.0;
...
datetime zeroTimeLong;
datetime zeroTimeShort;
...
int OnInit()
{
  ...
  highPrice = High[iHighest(thisSymbol, PERIOD_CURRENT, MODE_HIGH, Tick, 0)];
  lowPrice = Low[iLowest(thisSymbol, PERIOD_CURRENT, MODE_LOW, Tick, 0)];
...
bool orderLong(double lot) {
  ...
  double takeProfit1 = entryPrice + (widthToLaunch() * TP1_Width_To_Launch_Times) + (Buy_TP_Adjust_Pips * Point * 10.0);
  double takeProfit2 = entryPrice + (widthToLaunch() * TP2_Width_To_Launch_Times) + (Buy_TP_Adjust_Pips * Point * 10.0);

  double minSL = MarketInfo(Symbol(), MODE_STOPLEVEL) * Point;

  if(takeProfit1 - entryPrice < minSL) {
    Print("TP(", takeProfit1, ") is too close to entry point(", entryPrice, ") than minimum stoplevel(", minSL, ")");
    Print("Reconfigure parameters.");
    return False;
  }
  else if(takeProfit2 - entryPrice < minSL) {
    Print("TP(", takeProfit2, ") is too close to entry point(", entryPrice, ") than minimum stoplevel(", minSL, ")");
    Print("Reconfigure parameters.");
    return False;
  }
  else if(entryPrice - Ask < minSL && Entry_Wait_Tick == 0) {
    Print("Entering with Market Buy.");
    int ticket1 = OrderSend(thisSymbol, OP_BUY, lot, Ask, 3, NormalizeDouble(stopLoss, Digits), 0, NULL, Magic_Number);
    int ticket2 = OrderSend(thisSymbol, OP_BUY, lot, Ask, 3, NormalizeDouble(stopLoss, Digits), 0, NULL, Magic_Number);
    return True;
  }
    ...
    int ticket1 = OrderSend(thisSymbol, OP_BUYSTOP, lot, NormalizeDouble(entryPrice, Digits), 3, NormalizeDouble(stopLoss, Digits), NormalizeDouble(takeProfit1, Digits), NULL, Magic_Number);
    int ticket2 = OrderSend(thisSymbol, OP_BUYSTOP, lot, NormalizeDouble(entryPrice, Digits), 3, NormalizeDouble(stopLoss, Digits), NormalizeDouble(takeProfit2, Digits), NULL, Magic_Number);
...
bool orderShort(double lot) {
  ...
  double takeProfit1 = entryPrice - (widthToLaunch() * TP1_Width_To_Launch_Times) - (Sell_TP_Adjust_Pips * Point * 10.0);
  double takeProfit2 = entryPrice - (widthToLaunch() * TP2_Width_To_Launch_Times) - (Sell_TP_Adjust_Pips * Point * 10.0);

  double minSL = MarketInfo(Symbol(), MODE_STOPLEVEL) * Point;

  if(entryPrice - takeProfit1 < minSL) {
    Print("TP(", takeProfit1, ") is too close to entry point(", entryPrice, ") than minimum stoplevel(", minSL, ")");
    Print("Reconfigure parameters.");
    return False;
  }
  else if(entryPrice - takeProfit2 < minSL) {
    Print("TP(", takeProfit2, ") is too close to entry point(", entryPrice, ") than minimum stoplevel(", minSL, ")");
    Print("Reconfigure parameters.");
    return False;
  }
  ...
  else if(Bid - entryPrice < minSL && Entry_Wait_Tick == 0) {
    Print("Entering with Market Sell.");
    int ticket1 = OrderSend(thisSymbol, OP_SELL, lot, Bid, 3, NormalizeDouble(stopLoss, Digits), 0, NULL, Magic_Number);
    int ticket2 = OrderSend(thisSymbol, OP_SELL, lot, Bid, 3, NormalizeDouble(stopLoss, Digits), 0, NULL, Magic_Number);
    return True;
  }
    ...
    int ticket1 = OrderSend(thisSymbol, OP_SELLSTOP, lot, NormalizeDouble(entryPrice, Digits), 3, NormalizeDouble(stopLoss, Digits), NormalizeDouble(takeProfit1, Digits), NULL, Magic_Number);
    int ticket2 = OrderSend(thisSymbol, OP_SELLSTOP, lot, NormalizeDouble(entryPrice, Digits), 3, NormalizeDouble(stopLoss, Digits), NormalizeDouble(takeProfit2, Digits), NULL, Magic_Number);
...
void OnTick()
{
  ...
  if(buyOrderCount == 0) {
    ...
    if(zeroTimeLong == 0) {
      zeroTimeLong = TimeLocal() + (Entry_Wait_Tick * Period() * 60);
    }
  
    if(zeroTimeLong < TimeLocal()) {
      highPrice = High[iHighest(thisSymbol, PERIOD_CURRENT, MODE_HIGH, Tick, 0)];
      ObjectSetDouble(0, hLineID, OBJPROP_PRICE, highPrice);
  
      zeroTimeLong = (orderLong(calcLot()) ? 0 : zeroTimeLong);
    }
  }
  if(sellOrderCount == 0) {
  
    if(zeroTimeShort == 0) {
      zeroTimeShort = TimeLocal() + (Entry_Wait_Tick * Period() * 60);
    }
  
    if(zeroTimeShort < TimeLocal()) {
      lowPrice = Low[iLowest(thisSymbol, PERIOD_CURRENT, MODE_LOW, Tick, 0)];
      ObjectSetDouble(0, lLineID, OBJPROP_PRICE, lowPrice);
  
      zeroTimeShort = (orderShort(calcLot()) ? 0 : zeroTimeShort);
    }
  }
}
